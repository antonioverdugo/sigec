<x-app-layout>
    <div class="max-w-7xl mx-auto p-6 md:p-10 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-white tracking-tight">Crear un Patrocinador</h1>
                <p class="text-slate-400 mt-1">Introduce los datos y crea un patrocinador</p>
            </div>
        ...
        <div class="glass-panel p-8 rounded-2xl border border-slate-800">
            <form method="POST" action="{{ route('sponsors.store') }}" class="space-y-6">
                ...
                <!-- Telefono -->
                <div>
                    <label for="phone" class="block text-sm font-medium text-slate-300 mb-2">Teléfono (opcional)</label>
                ...
                <div>
                    ...
                    <input type="number" name="amount_contributed" id="amount_contributed"
                        ...
                        oninput="this.setCustomValidity('')"
                        class="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                        placeholder="Valores numéricos entre 0 y 999999999"
                        value="{{old('amount_contributed')}}">
                ...
                <div>
                    <label for="type_sponsor_id" class="block text-sm font-medium text-slate-300 mb-2">Tipo de Patrocinador *</label>
                    <select name="type_sponsor_id" id="role_id" required
                        ...
                        @foreach($typeSponsors as $typeSponsor)
                            <option value="{{ $typeSponsor->id }}" class="bg-slate-800" {{ old('type_sponsor_id') == $typeSponsor->id ? 'selected' : '' }}>{{ ucwords($typeSponsor->name) }}</option>
                        @endforeach
                    </select>
