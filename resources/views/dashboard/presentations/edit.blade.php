<x-app-layout>
    <div class="max-w-7xl mx-auto p-6 md:p-10 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-white tracking-tight">Editar una Ponencia</h1>
                <p class="text-slate-400 mt-1">Introduce los datos y edita la ponencia</p>
            </div>
        ...
        <div class="glass-panel p-8 rounded-2xl border border-slate-800">
            <form method="POST" action="{{ route('presentations.update', ['presentation' => $presentation]) }}" class="space-y-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" required value="{{ old('title', $presentation->title) }}"
                ...
                <div>
                    ...
                    <textarea
                      ...
                      class="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      required
                    > {{ old('summary', $presentation->summary) }}</textarea>
                    @error('summary')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="file" class="block text-sm font-medium text-slate-300 mb-2">Subir Archivo Ponencia (opcional si no se elige un archivo se mantendrá el subido anteriormente)</label>
